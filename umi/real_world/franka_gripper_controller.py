...
from umi.real_world.franka_gripper_interface import FrankaGripperInterface
...
class FrankaGripperController(mp.Process):
    ...
    def run(self):
        # start connection
        try:
            gripper = FrankaGripperInterface()
                
            # home gripper to initialize
            gripper.open()

            # get initial
            curr_pos = gripper.get_pos()
            # curr_pos = 100.0
            ...
            while keep_running:
                ...
                gripper.set_speed(target_vel)
                gripper.set_pos(target_pos)
                # time.sleep(1e-3)

                # get state from robot
                state = {
                    'gripper_position': gripper.get_pos() / self.scale,
                    'gripper_receive_timestamp': time.time(),
